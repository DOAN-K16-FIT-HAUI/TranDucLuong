...
class _GroupNoteScreenState extends State<GroupNoteScreen> {
  ...
  final GroupNoteRepository _groupNoteRepo =
      GetIt.instance<GroupNoteRepository>();
            ...
            FirebaseFirestore.instance
                ...
                .then((groupDoc) {
                  if (groupDoc.exists) {
                    ...
                    debugPrint(
                      "Group document $groupId not found for user $userId's membership.",
                    );
                  }
                })
                .catchError((e) {
      ...
      if (mounted) {
        setState(() {
          ...
          if (groups.isNotEmpty) {
            _selectedGroupId =
                groups.first['groupId']; // Select the first group after sorting
      ...
      appBar: AppBarTabBar.buildAppBar(
        ...
        actions: [
          if (_userGroups.isNotEmpty && _selectedGroupId != null) ...[
            IconButton(
              icon: Icon(
                groupNoteBloc.state.isSearching ? Icons.close : Icons.search,
              ),
              tooltip:
                  groupNoteBloc.state.isSearching
                      ? l10n.closeSearchTooltip
                      : l10n.searchTooltip,
              onPressed:
                  () => groupNoteBloc.add(
                    ToggleSearch(!groupNoteBloc.state.isSearching),
                  ),
            ),
            IconButton(
                            ...
                            if (state.isSearching)
                              Padding(
                                ...
                                child: UtilityWidgets.buildSearchField(
                                  ...
                                  onChanged:
                                      (value) =>
                                          groupNoteBloc.add(SearchNotes(value)),
                                ),
                              ),
                            Padding(
                              padding: const EdgeInsets.symmetric(
                              ...
                              child:
                                  notesToShow.isEmpty
                                        ...
                                        child: UtilityWidgets.buildEmptyState(
                                          ...
                                          message:
                                              state.isSearching ||
                                                  ...
                                                  ? l10n
                                                      .noNotesFoundSearchFilter
                                                  : l10n.noNotesInGroup,
                                          suggestion:
                                              state.isSearching ||
                                                      state.selectedTag != null
                                                  ? l10n
                                                      .tryDifferentSearchFilter
                                      ...
                                      : ListView.builder(
                                        ...
                                        // Thêm padding cần thiết
                                        itemCount: notesToShow.length,
                                        itemBuilder:
                                            (ctx, index) => _buildNoteCard(
                                              ctx,
                                              notesToShow[index],
                                              index,
                                            ),
  ...
  void _navigateToAddEditNote(BuildContext context, GroupNoteModel? note) {
    AppRoutes.navigateToAddEditGroupNote(
      ...
      (savedNote) {
        ...
        if (note == null) {
          bloc.add(AddNote(savedNote));
        } else {
          bloc.add(EditNote(savedNote));
        }
      },
  ...
  Widget _buildNoteCard(BuildContext context, GroupNoteModel note, int index) {
    ...
    if (note.tags.contains(l10n.tagExpense)) {
      cardIcon = Icons.receipt_long_outlined;
    } else if (note.tags.contains(l10n.tagGoal)) {
      cardIcon = Icons.flag_outlined;
    }
    ...
    return ListsCards.buildItemCard(
      ...
      onMenuSelected: (result) {
        MenuActions.handleEditDeleteActions<GroupNoteModel>(
          ...
          onDelete:
              (ctx, item) => ctx.read<GroupNoteBloc>().add(
